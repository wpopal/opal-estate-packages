...
if ( version_compare( $GLOBALS['wp_version'], '4.6', '<' ) ) {
	 ...
	 */
	function opal_estate_packages_wordpress_upgrade_notice() {
		...
		deactivate_plugins( [ 'opal_estate_packages/opal_estate_packages.php' ] );
	}

	add_action( 'admin_notices', 'opal_estate_packages_wordpress_upgrade_notice' );
...
if ( version_compare( phpversion(), '5.6', '<' ) ) {
	 ...
	 */
	function opal_estate_packages_php_upgrade_notice() {
	...
	add_action( 'admin_notices', 'opal_estate_packages_php_upgrade_notice' );

	return;
}

if ( defined( 'OPAL_ESTATE_PACKAGES_VERSION' ) ) {
	return;
}

define( 'OPAL_ESTATE_PACKAGES_VERSION', '1.0.0' );
define( 'OPAL_ESTATE_PACKAGES_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
define( 'OPAL_ESTATE_PACKAGES_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

/**
 * Admin notice: Require WooCommerce.
 */
function opal_estate_packages_admin_notice() {
...
add_action( 'plugins_loaded', function () {
	if ( class_exists( 'WooCommerce' ) ) {
		$GLOBALS['opal_estate_packages'] = Opal_Estate_Packages::get_instance();
	}
	add_action( 'admin_notices', 'opal_estate_packages_admin_notice', 4 );
} );
